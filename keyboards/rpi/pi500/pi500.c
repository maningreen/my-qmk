...
bool shutdown_kb(bool jump_to_bootloader) {
    ...
    // Set all pins to input pull down on shutdown to prevent triggering the power button after soft reset
    const uint8_t matrix_col_pins[] = MATRIX_COL_PINS;
    const uint8_t matrix_row_pins[] = MATRIX_ROW_PINS;
    for (int r = 0; r < MATRIX_ROWS; r++) {
        gpio_set_pin_input_low(matrix_row_pins[r]);
    }
    for (int c = 0; c < MATRIX_COLS; c++) {
        gpio_set_pin_input_low(matrix_col_pins[c]);
    }
    return true;
}
